...
class PowerballController extends Controller
{
	public function actionIndex()
	{
		...
		$code = $query
			->from('qishu')
			->one();
		$temp = json_decode($this->_request($code['qishu']), true);
		if ($temp['state'] != 0) {
			echo "没有数据".$code['qishu'];
			return;
		}else{
	 ...
	 * @return [type] [description]
	 */
	public function _request($code)
	{
		$refer = 'http://www.cwl.gov.cn/kjxx/ssq/';
		$ch = curl_init();
		//设置是直接打印出来 ture不打印
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_URL, 'http://www.cwl.gov.cn/cwl_admin/kjxx/findKjxx/forIssue?name=ssq&code='.$code);
		//伪造来源refer
